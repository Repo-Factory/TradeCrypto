...
const bool BROKER_FULL(Producer* producer_context)      
{
    ...
    const bool broker_full = producer_context->broker.size() >= MAX_CRYPTO_REQUESTS;
...
{
    pthread_mutex_lock(&producer_context->broker_mutex);
    const bool bitcoin_full = SharedData::getQueueData(producer_context->broker)[Requests::Bitcoin] >= MAX_BITCOIN_REQUESTS;
    pthread_mutex_unlock(&producer_context->broker_mutex);
    return bitcoin_full;
}
...
void* ProducerThread::produce(void* arg)
{
    ...
    while (STILL_REQUESTS(producer_context))
    {
        while (BROKER_FULL(producer_context)) {
            Threading::safeAction(&producer_context->general_mutex, [&](){
                pthread_cond_wait(&producer_context->general_monitor, &producer_context->general_mutex);
            });
        } 
        while (BITCOIN_FULL(producer_context) && producer_context->request_type == Requests::Bitcoin) {
            Threading::safeAction(&producer_context->bitcoin_mutex, [&](){
                pthread_cond_wait(&producer_context->bitcoin_monitor, &producer_context->bitcoin_mutex);
            });
        }
